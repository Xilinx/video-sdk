...
# Stop if the XRM daemon, if it is running
...
xbutil examine --format json --output "${temp_json_file}" --force 2>&1 > "${temp_log_file}"
...
for device in devices:
  ...
  print("\n=====================================================")
  print("[%d]: Running xbutil validate for device: %s" %  (device_count, device["bdf"]))
  print("=====================================================")
  cmd = "xbutil validate --device " + device["bdf"]
  print("%s" % cmd)
  os.system(cmd)
