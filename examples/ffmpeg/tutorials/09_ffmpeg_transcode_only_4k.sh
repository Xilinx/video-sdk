...
# You may change the target codecs by changing:
#     the input codec from  (-c:v mpsoc_vcu_hevc) to (-c:v mpsoc_vcu_h264) for decoding from H264
#     the output codec from (-c:v mpsoc_vcu_h264) to (-c:v mpsoc_vcu_hevc) for encoding to HEVC

if [ $# -ne 1 ]
  then
    echo "Incorrect arguments supplied"
    echo "Usage: $(basename $0) <2160p60 HEVC clip>"
    exit 1
fi
ffmpeg -c:v mpsoc_vcu_hevc -i $1 \
