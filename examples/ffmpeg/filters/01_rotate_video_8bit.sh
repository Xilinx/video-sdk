...
if [ $# -ne 2 ]
  then
    ...
    echo "Usage: ./01_rotate_video_8bit.sh <1080p60 h264 clip> <direction {0..3}>"
    exit 1
fi
...
if [ $bps != "bits_per_raw_sample=8" ]
  then
    ...
    echo "The 01_rotate_video_8bit.sh script only supports 8bit streams."
    echo "Use 01_rotate_video_10bit.sh for 10bit streams."
    exit 1
fi
