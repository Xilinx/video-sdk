...
if [ $# -ne 2 ]
  then
    ...
    echo "Usage: ./02_logo_overlay_8bit.sh <h264 clip> <Logo.png>"
    exit 1
fi
...
if [ $bps != "bits_per_raw_sample=8" ]
  then
    ...
    echo "The 02_logo_overlay_8bit.sh script only supports 8bit streams."
    echo "Use 02_logo_overlay_10bit.sh for 10bit streams."
    exit 1
fi
