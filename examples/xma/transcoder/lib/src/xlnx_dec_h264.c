...
/**
 * xlnx_dec_parse_h264_scal_list: Parse H.264 scaling list
 * @param sizeoflist: size of the scaling list
 * @param getbits: structure to read from
 * @return XMA_APP_SUCCESS on success
 */
static int32_t xlnx_dec_parse_h264_scal_list(int32_t         sizeoflist,
 ...
 */
static int32_t xlnx_dec_parse_h264_sps(int32_t file, XlnxDecBuffer* dec_buffer,
                                       ...
                                       int32_t           startoffset)
{
    ...
    uint8_t seq_param_set_id = xlnx_dec_get_bits_unsigned_eg(&getbits);
    ...
    if((sps.profile_idc == 100) || (sps.profile_idc == 110) ||
       (sps.profile_idc == 122) || (sps.profile_idc == 144)) {
        ...
        parsedata->luma_bit_depth =
            (xlnx_dec_get_bits_unsigned_eg(&getbits) + 8);
        ...
        if(seq_scaling_matrix_present_flag) {
            for(i = 0; i < 8; i++) {
                ...
                if(seq_scal_list_present_flag[i]) {
                    if(i < 6) {
                        xlnx_dec_parse_h264_scal_list(16, &getbits);
                    } else {
                        xlnx_dec_parse_h264_scal_list(64, &getbits);
                    }
                }
            }
        }
    ...
    uint8_t vui_parameters_present_flag = xlnx_dec_get_bits_byte(&getbits, 1);
    if(vui_parameters_present_flag) {
        uint8_t aspect_ratio_info_present_flag =
            xlnx_dec_get_bits_byte(&getbits, 1);
        if(aspect_ratio_info_present_flag) {
            uint8_t aspect_ratio_idc = xlnx_dec_get_bits_byte(&getbits, 8);
    ...
    if(sps.frame_cropping_flag) {
        int32_t crop_unitx = 0;
        int32_t crop_unity = 0;
    ...
    sps.valid                                           = 1;
    parsedata->h264_seq_parameter_set[seq_param_set_id] = sps;
                                       ...
                                       int32_t           startoffset)
{
    ...
    uint8_t pic_parameter_set_id = xlnx_dec_get_bits_unsigned_eg(&getbits);
    pps.seq_param_set_id         = xlnx_dec_get_bits_unsigned_eg(&getbits);
 ...
 * @param dec_buffer: Decoder input buffer
 * @param parsedata: structure for parsed data
 * @param sliceheader: Slice header structure
 * @param startoffset: Current offset for the input buffer
 * @return XMA_APP_SUCCESS on success, XMA_APP_ERROR on error, or
 * DEC_INPUT_EOF if the end of the input file is reached
 */
static int32_t xlnx_dec_parse_h264_slice_header(
    int32_t file, XlnxDecBuffer* dec_buffer, XlnxDecFrameData* parsedata,
    XlnxH264SliceHeader* sliceheader, int32_t startoffset)
{
    int32_t endoffset;
    int32_t ret = xlnx_dec_find_next_start_code(file, dec_buffer,
                                                startoffset + 1, &endoffset);
    ...
    XlnxH264PicParamSet pps =
        parsedata->h264_pic_parameter_set[sliceheader->pic_parameter_set_id];
    if(!pps.valid) {
        free(buffer.data);
        xma_logmsg(XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
                   "Got Error in slice header %d\n", __LINE__);
    ...
    XlnxH264SeqParamSet sps =
        parsedata->h264_seq_parameter_set[pps.seq_param_set_id];
    if(!sps.valid) {
        free(buffer.data);
        xma_logmsg(XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
                   "Got Error in slice header %d\n", __LINE__);
    ...
    sliceheader->frame_num =
        xlnx_dec_get_bits_short(&getbits, sps.log2_max_frame_num_minus4 + 4);
    ...
    if(sps.pic_order_cnt_type == 0) {
        sliceheader->pic_order_cnt_lsb = xlnx_dec_get_bits_short(
            &getbits, sps.log2_max_pic_order_cnt_lsb_minus4 + 4);
                               ...
                               XlnxDecFrameData* parsedata, int32_t* retoffset)
{
    ...
    while(true) {
        int32_t endoffset = 0;
        ret = xlnx_dec_find_next_start_code(file, dec_buffer, startoffset,
                                            &endoffset);
        if(ret == XMA_APP_ERROR) {
            xma_logmsg(XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
        ...
        if(dec_buffer->size <= (uint32_t)(endoffset + 4)) {
        ...
        uint8_t nalutype = dec_buffer->data[endoffset + 3] & 0x1F;
        if(nalutype == 7)
            ret =
                xlnx_dec_parse_h264_sps(file, dec_buffer, parsedata, endoffset);
        if(nalutype == 8)
            ret =
                xlnx_dec_parse_h264_pps(file, dec_buffer, parsedata, endoffset);
        ...
        if((1 <= nalutype) && (nalutype <= 5)) {
            ...
            XlnxH264SeqParamSet sps =
                parsedata->h264_seq_parameter_set
                    [parsedata
                         ->h264_pic_parameter_set[sliceheader
                                                      .pic_parameter_set_id]
                         .seq_param_set_id];
