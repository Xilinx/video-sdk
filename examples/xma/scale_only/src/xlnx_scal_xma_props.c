...
Parameters:
    param_ctx: The parameter context containing scaler param values
    session_id: The session id corresponding to the current session
    scaler_xma_props: The scaler xma properties which will be filled
Return:
                                       ...
                                       XmaScalerProperties* scaler_xma_props) 
{
    ...
    scaler_xma_props->params[3].length = sizeof(param_ctx->
                                                mixrate_session_as_uint64);
                                      ...
                                      XmaScalerProperties* scaler_xma_props) 
{
    ...
    scaler_xma_props->num_outputs                  = param_ctx->
                                                     num_outputs[session_id];
    scaler_xma_props->input.format                 = param_ctx->input_pix_fmt;
    scaler_xma_props->input.bits_per_pixel         = param_ctx->
    ...
    scaler_xma_props->params      = calloc(1, MAX_SCALER_PARAM * 
                                           sizeof(XmaParameter));
    if(session_id > 0) {
    ...
    for(int output_id = 0; output_id < param_ctx->num_outputs[0]; output_id++) {
        ...
        scaler_xma_props->output[prop_index].bits_per_pixel = 8;
    ...
    for(int i = 0; i < param_ctx->num_outputs[0]; i++) {
...
Return:
    SCALER_APP_SUCCESS on success
------------------------------------------------------------------------------*/
int32_t xlnx_scal_create_xma_props(XlnxScalerProps* param_ctx,
                                    int session_id,
                                    XmaScalerProperties* scaler_xma_props)
{
    // Setup custom xma plugin params
    xlnx_scal_create_props(param_ctx, session_id, scaler_xma_props);
    xlnx_scal_create_params(param_ctx, session_id, scaler_xma_props);
    return SCALER_APP_SUCCESS;
}
