...
pipe="gst-launch-1.0 -v filesrc location=$INPUT ! h264parse ! vvas_xvcudec  dev-idx=$di ! video/x-raw, format=NV12 ! videoflip video-direction=${DIRECTION} ! vvas_xvcuenc  dev-idx=$di target-bitrate=8000 ! h264parse ! qtmux ! fpsdisplaysink video-sink=\"filesink location=/tmp/xil_rotate_${DIRECTION}.mp4\" sync=false text-overlay=false"
