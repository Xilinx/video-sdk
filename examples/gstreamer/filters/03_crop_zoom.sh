...
pipe="gst-launch-1.0 -v filesrc location=$INPUT ! h264parse ! vvas_xvcudec   dev-idx=$di ! videocrop top=$YPOSITION bottom=$BOTTOM left=$XPOSITION right=$RIGHT ! vvas_xvcuenc  dev-idx=$di target-bitrate=8000 ! h264parse ! qtmux ! fpsdisplaysink video-sink=\"filesink location=/tmp/xil_crop_zoom.mp4\" sync=false text-overlay=false"
