...
pipe="filesrc location=$2 ! rawvideoparse format=nv12 width=1920 height=1080 framerate=60/1 ! vvas_xlookahead codec-type=1 lookahead-depth=20 spatial-aq=true temporal-aq=true dev-idx=$di b-frames=1 ! vvas_xvcuenc dev-idx=$di b-frames=1 gop-length=120 periodicity-idr=120 qp-mode=3 target-bitrate=$3 max-bitrate=$3 ! h265parse ! fpsdisplaysink video-sink=\"filesink location=/tmp/xil_enc_${bitrate}_subjective.h265 \" text-overlay=false sync=false "$pipe
