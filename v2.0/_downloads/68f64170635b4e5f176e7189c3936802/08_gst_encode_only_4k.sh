...
if [ $# -lt 3 ]; then
     ...
     echo "NOTE: For input raw stream resolutions other than 4k, change width, height, blocksize and framerate params in the script accordingly"
...
pipe="filesrc location=$2 blocksize=$blocksize
	...
	! vvas_xvcuenc dev-idx=$di target-bitrate=20000 max-bitrate=20000 num-slices=4 enable-pipeline=true
	! h264parse
	${muxer}
