...
if [ $# -ne 4 ]
  then
    ...
    echo "Usage: ./04_multi_comp_8bit.sh <h264 clip> <h264 clip> <h264 clip> <h264 clip>"
    exit 1
fi
...
if [ $bps != "bits_per_raw_sample=8" ]
  then
    ...
    echo "The 04_multi_comp_8bit.sh script only supports 8bit streams."
    echo "Use 04_multi_comp_10bit.sh for 10bit streams."
    exit 1
fi
