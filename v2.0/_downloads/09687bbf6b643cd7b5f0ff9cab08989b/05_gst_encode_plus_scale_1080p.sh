...
if [ $# -lt 3 ]; then
     ...
     echo "NOTE: For input raw stream resolutions other than 1080p, change width, height, blocksize and framerate params in the script accordingly"
...
pipe="gst-launch-1.0 filesrc location=$2 blocksize=$blocksize
	...
	! queue
	! vvas_xabrscaler dev-idx=$di ppc=4 scale-mode=2 name=$sc_name avoid-output-copy=true enable-pipeline=true
	$sc_name.src_0
	    ! queue
		...
		! vvas_xvcuenc name=$enc_720p60 dev-idx=$di  target-bitrate=4000 max-bitrate=4000
		! h264parse
               ${muxer}
		...
		! vvas_xvcuenc name=$enc_720p30 dev-idx=$di  target-bitrate=3000 max-bitrate=3000
		! h264parse
               ${muxer}
	...
	$sc_name.src_1
	    ...
	    ! videorate
	    ! video/x-raw, framerate=30/1
	    ! vvas_xvcuenc name=$enc_480p30 dev-idx=$di  target-bitrate=2500 max-bitrate=2500
	    ! h264parse
           ${muxer}
	...
	$sc_name.src_2
	    ...
	    ! videorate
	    ! video/x-raw, framerate=30/1
	    ! vvas_xvcuenc name=$enc_360p30 dev-idx=$di  target-bitrate=1250 max-bitrate=1250
	    ! h264parse
           ${muxer}
	...
	$sc_name.src_3
	    ...
	    ! videorate
	    ! video/x-raw, framerate=30/1
	    ! vvas_xvcuenc name=$enc_160p30 dev-idx=$di  target-bitrate=625 max-bitrate=625
	    ! h264parse
           ${muxer}
