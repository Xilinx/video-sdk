...
codec_type=`gst-discoverer-1.0 $2 | grep "video codec:" | cut -d":" -f2 | awk '{print $1;}'`
echo $container
echo $codec_type
...
pipe="gst-launch-1.0 filesrc num-buffers=$num location=$input_file
	${demuxer}
	! h264parse
	! vvas_xvcudec  dev-idx=$di low-latency=1
	! queue
	! vvas_xabrscaler  dev-idx=$di ppc=4 scale-mode=2 avoid-output-copy=true name=$sc_name 
	$sc_name.src_0
	    ! queue
		...
		! vvas_xvcuenc name=$enc_720p60 dev-idx=$di  target-bitrate=4000 b-frames=0 scaling-list=0
		! h264parse
		${muxer}
		...
		! vvas_xvcuenc name=$enc_720p30 dev-idx=$di  target-bitrate=3000 b-frames=0 scaling-list=0
		! h264parse
		${muxer}
	...
	$sc_name.src_1
	    ...
	    ! videorate
	    ! video/x-raw, framerate=30/1
	    ! vvas_xvcuenc name=$enc_480p30 dev-idx=$di  target-bitrate=2500 b-frames=0 scaling-list=0
	    ! h264parse
	    ${muxer}
	...
	$sc_name.src_2
	    ...
	    ! videorate
	    ! video/x-raw, framerate=30/1
	    ! vvas_xvcuenc name=$enc_360p30 dev-idx=$di  target-bitrate=1250 b-frames=0 scaling-list=0
	    ! h264parse
	    ${muxer}
	...
	$sc_name.src_3
	    ...
	    ! videorate
	    ! video/x-raw, framerate=30/1
	    ! vvas_xvcuenc name=$enc_160p30 dev-idx=$di  target-bitrate=625 b-frames=0 scaling-list=0
	    ! h264parse
	    ${muxer}
