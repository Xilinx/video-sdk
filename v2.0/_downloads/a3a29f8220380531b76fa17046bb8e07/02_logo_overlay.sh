...
pipe="gst-launch-1.0 -v filesrc location=$INPUT  ! h264parse ! vvas_xvcudec   dev-idx=$di ! gdkpixbufoverlay location=$LOGO overlay-height=100 overlay-width=500 offset-x=16 offset-y=16 ! vvas_xvcuenc  dev-idx=$di target-bitrate=8000 ! h264parse ! qtmux ! fpsdisplaysink video-sink=\"filesink location=/tmp/xil_logo_overlay.mp4\" sync=false text-overlay=false"
